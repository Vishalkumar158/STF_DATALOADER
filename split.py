...
#             pts_img = pts_rect[:, :2] / pts_rect[:, 2:3]
...
#     h, w = img_shape
#     in_x = np.logical_and(pts_img[:, 0] >= 0, pts_img[:, 0] < w)
#     in_y = np.logical_and(pts_img[:, 1] >= 0, pts_img[:, 1] < h)
#     in_front = pts_depth > 0
#     return np.logical_and(np.logical_and(in_x, in_y), in_front)
...
#     if ',' in stem:
#         date_time, frame_num = stem.split(',', 1)
#     else:
#         parts = stem.rsplit('_', 1)
#         date_time = parts[0]
#         frame_num = parts[1]
#     return date_time, frame_num.zfill(5)
...
# for f in bin_files:
#     date_time, frame_num = extract_parts(f.stem)
#     formatted = f"{date_time}, {frame_num}"
#     formatted_ids.append(formatted)
#     raw_stems.append(f.stem)  
...
# train_formatted = [formatted_ids[i] for i in train_indices]
# val_formatted = [formatted_ids[i] for i in val_indices]

# print(f"Initial split: {len(train_formatted)} train, {len(val_formatted)} val")

# with open(OUTPUT_DIR / 'train.txt', 'w') as f:
#     for line in train_formatted:
#         f.write(line + '\n')

# with open(OUTPUT_DIR / 'val.txt', 'w') as f:
#     for line in val_formatted:
#         f.write(line + '\n')

# print("Saved initial train.txt and val.txt")

# for split_name, fmt_ids, idx_list in [('train', train_formatted, train_indices),
#                                      ('val', val_formatted, val_indices)]:
#     print(f"\nFiltering {split_name.upper()} split ({len(fmt_ids)} frames)...")
#     good_formatted = []
#     removed = 0

#     for i, formatted_id in enumerate(tqdm(fmt_ids)):
#         original_stem = raw_stems[idx_list[i]]  
#         bin_file = LIDAR_FOLDER / f'{original_stem}.bin'
...
#             good_formatted.append(formatted_id)
#         else:
#             removed += 1

#     print(f"→ Kept {len(good_formatted)} frames (>= {MIN_FOV_POINTS} FOV points)")
#     print(f"→ Removed {removed} frames")

#     # Save final filtered split
#     final_file = OUTPUT_DIR / f'{split_name}.txt'
#     with open(final_file, 'w') as f:
#         for line in good_formatted:
#             f.write(line + '\n')

#     print(f"Final clean split saved: {final_file}")
